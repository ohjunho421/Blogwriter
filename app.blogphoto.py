...
if st.button("생성하기"):
    with st.spinner('생성 중입니다...'):
        ...
        6. 효과를 설명하는 소주제 내용을 작성할 때 정량적인 지표를 넣어주고 아래 예시를 사용해서 문맥상 자연스럽게 작성해줘:
            ...
            - 3. 출처가 없다면 작성하지 않거나 관련된 기사를 언급해줘.
        ...
        # DALL-E 이미지 생성 호출 (20장 반복 생성)
        images = []
        for i in range(20):
            image_response = openai.Image.create(
                prompt=f"{story1}, {story2}, {story3}, {story4}, {story5}와 관련된 고해상도 현실적인 사진",
                size="1024x1024",
            )
    ...
    for idx, image_url in enumerate(images, start=1):
        st.image(image_url, caption=f"이미지 {idx}")